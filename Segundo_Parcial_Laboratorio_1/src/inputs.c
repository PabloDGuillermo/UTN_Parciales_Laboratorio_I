...
/// @fn int utn_pedirTarjetaDeCredito(char*, char*, char*, int)
/// @brief Pide al usuario el ingreso de uns tarjeta de credito de 16 digitos
/// @param pResultado Puntero a la variable que se quiere escribir
/// @param reintentos Cantidad posible de reintentos
/// @return Retorna 0 si los datos ingresados son correctos y -1 si no lo son
int utn_pedirTarjetaDeCredito(char *pResultado, int reintentos) {
	char bufferTexto[17];
	int retorno = -1;
	while (reintentos > 0) {
		reintentos--;
		...
		if (myGets(bufferTexto, sizeof(bufferTexto)) == 0
				&& strlen(bufferTexto) == 16 && esNumerica(bufferTexto)) {
			strcpy(pResultado, bufferTexto);
			retorno = 0;
			break;
		}
		printf("ERROR. Tarjeta de credito invalida\n");
	}
	return retorno;
}
...
int utn_pedirFecha(int *pDia, int *pMes, int *pAnio, int reintentos) {
	int retorno;
	int error;

	retorno = -1;

	error = utn_pedirNumero(pAnio, "Ingrese el año: ", "ERROR. Año invalido.",
			2000, 2023, 3);
	if (error == 0) {
		error = utn_pedirNumero(pMes, "ingrese el mes: ",
				"ERROR. Mes invalido.", 1, 12, 3);
		if (error == 0
				&& (*pMes == 1 || *pMes == 3 || *pMes == 5 || *pMes == 7
						|| *pMes == 8 || *pMes == 10 || *pMes == 12)) {
			utn_pedirNumero(pDia, "Ingrese el dia: ",
